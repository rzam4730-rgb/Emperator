...
def mount_core_routes(app, conn, current_user, require_permission, hash_password, verify_password, create_access_token, issue_refresh_token, hash_refresh_token, ensure_subscription, plan_limits, get_usage, utcnow, audit):
    @app.post("/api/auth/login")
    def login(payload: dict):
        phone = str(payload.get("phone", "")).strip(); password = str(payload.get("password", "")); c = conn()
        ...
        ensure_subscription(c, row["restaurant_id"]); access = create_access_token(row["id"], row["restaurant_id"], row["role"]); refresh = issue_refresh_token(c, row["id"], row["restaurant_id"]); c.commit(); c.close()
    ...
    def refresh(payload: dict):
        ...
        c.execute("UPDATE refresh_tokens SET revoked_at=? WHERE id=?", (datetime.now(timezone.utc).isoformat(), row["id"])); new_refresh = issue_refresh_token(c, user["id"], row["restaurant_id"]); access = create_access_token(user["id"], row["restaurant_id"], user["role"]); c.commit(); c.close()
    ...
    def list_plans(user=Depends(current_user)):
        c = conn(); rows=[]
        for p in c.execute("SELECT * FROM plans WHERE active=1 ORDER BY price_monthly").fetchall(): rows.append({"code":p["code"],"name":p["name"],"price_monthly":p["price_monthly"],"limits":plan_limits(p)})
        c.close(); return rows

    @app.get("/api/subscription")
    def subscription(user=Depends(current_user)):
        c=conn(); s=ensure_subscription(c,user["restaurant_id"]); rid=user["restaurant_id"]; customers=c.execute("SELECT COUNT(*) FROM customers WHERE restaurant_id=?",(rid,)).fetchone()[0]
    ...
    def list_roles(user=Depends(require_permission("users.manage"))):
        ...
        for r in c.execute("SELECT id,name FROM roles ORDER BY id").fetchall():
            perms=[x[0] for x in c.execute("SELECT p.name FROM permissions p JOIN role_permissions rp ON rp.permission_id=p.id WHERE rp.role_id=? ORDER BY p.name",(r["id"],)).fetchall()]; rows.append({"id":r["id"],"name":r["name"],"permissions":perms})
        c.close(); return rows
    ...
    def create_user(payload:dict,user=Depends(require_permission("users.manage"))):
        ...
        try:
            uid=c.execute("INSERT INTO users(name,phone,password_hash,created_at) VALUES(?,?,?,?)",(name,phone,hash_password(password),datetime.now(timezone.utc).isoformat())).lastrowid; c.execute("INSERT INTO user_restaurants(user_id,restaurant_id,role_id) VALUES(?,?,?)",(uid,user["restaurant_id"],role["id"])); audit(c,user,"user.create","user",uid,f"role={role_name}"); c.commit()
    ...
    def update_user(user_id:int,payload:dict,user=Depends(require_permission("users.manage"))):
        ...
        try:
            ...
            if "role" in payload:
                role_name=str(payload.get("role","cashier")).strip().lower(); role=c.execute("SELECT id,name FROM roles WHERE name=?",(role_name,)).fetchone()
            ...
            audit(c,user,"user.update","user",user_id,str(payload)); c.commit()
            row=c.execute("SELECT u.id,u.name,u.phone,u.status,r.name role FROM users u JOIN user_restaurants ur ON ur.user_id=u.id JOIN roles r ON r.id=ur.role_id WHERE u.id=? AND ur.restaurant_id=?",(user_id,user["restaurant_id"])).fetchone(); return dict(row)
        except sqlite3.IntegrityError:
            c.rollback(); raise HTTPException(409,"این شماره موبایل قبلاً ثبت شده است")
        finally: c.close()
    ...
    def audit_logs(user=Depends(require_permission("users.manage"))):
        c=conn(); rows=[dict(x) for x in c.execute("SELECT id,action,target_type,target_id,details,created_at FROM audit_logs WHERE restaurant_id=? ORDER BY id DESC LIMIT 50",(user["restaurant_id"],))]; c.close(); return rows
